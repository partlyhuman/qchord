...
for (int offset = 0; offset < buffer.Length;)
{
    ...
    switch (chunk.Header.Id)
    {
        ...
        case "PARA":
        {
            ReadOnlySpan<InstrumentSplit> splits = InstrumentSplit.TakeN(chunk.Data);
            foreach (InstrumentSplit split in splits) Console.WriteLine(split);
            ...
            File.WriteAllBytes(dumpPath, MemoryMarshal.Cast<InstrumentSplit, byte>(splits).ToArray());
